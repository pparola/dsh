...
const columns=[
  ...
  { title: "Capital", field: "Capital"  ,width:115 ,type:"currency"},
  { title: "Interes", field: "Interes"   ,width:115 ,type:"currency"},
  { title: "Total", field: "Total"   ,width:115 ,type:"currency"},
...
const d=new Date();
const title="Venta Operaciones Diarias";
...
const totalPorDia = (data) =>{
  ...
  if (data.length>0){
    ...
    while (i<data.length )
    {
      ...
      let tInteres=0;
      let tTotal=0;
      newreg.push({Id:kdia,Comercio:"", Nombre:"",Dia: kdia, Cantidad:0, Capital:0, Interes:0, Total:0});
      let j=newreg.length -1;
      while  (i<data.length && kdia==data[i].Dia)
      {
        ...
        newreg.push({
          Id: kdia*10000 + Number(data[i].ComercioCodigo),
          Comercio:data[i].ComercioCodigo,
          Nombre:data[i].ComercioNombre,
          Dia:data[i].Dia,
...
const handleOnClickOk = (e , p) => {
  ...
  axios.get( getUri('VentaOperacionesDiarias') , {
    params: {
      year: p.year,
      month: p.month,
      cartera: p.cartera

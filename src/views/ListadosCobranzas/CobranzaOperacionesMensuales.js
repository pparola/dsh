...
export default function CobranzaOperacionesMensuales() {
  const classes = useStyles();
...
const d=new Date();
const title="Cobranza Operaciones Mensuales";
const tableRef = React.createRef();
const putData = (data) => {
  let resul=totaliza(data);
  console.log(resul);
...
const totaliza = (data) =>{
  ...
  if (data.length>0){
    ...
    while (i<data.length )
    {
      ...
      let tRecargo=0;
      let tTotal=0;
      newreg.push({Id:key,Comercio:"", Nombre:"",Mes:dateNames.monthMin[key-1], Cantidad:0, Importe:0, Recargo:0, Total:0});
...
const handleOnClickOk = (e ,p) => {
  ...
  axios.get(getUri('CobranzaOperacionesMensuales'), {
    params: {
      year: p.year,
      cartera: p.cartera
    }
